 ...
 *
 * Most operations involving HTTP::StatusCode are also doable using a plain
 * integer status code.
 */
...
StatusCode
toStatusCode(unsigned intStatusCode);
unsigned
toStatusCode(StatusCode statusCode);
std::string
toStatusCodeString(StatusCode statusCode);
std::string
toStatusCodeString(unsigned intStatusCode);
}