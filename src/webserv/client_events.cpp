...
{
    for (std::map<int, HTTP::Request>::iterator it = _reqs.begin();
         it != _reqs.end();) {
        ...
        if (FD_ISSET(csockfd, &rset)) {
            ...
            if (req.getState() == HTTP::Request::W4_HEADER) {
                ...
                if ((pos = req.data.str().find(HTTP::BODY_DELIMITER)) !=
                    ...
                    ConfigItem* serverBlock = _selectServer(
            ...
            if (FD_ISSET(csockfd, &wset)) {
                ...
                // NOTE: this is a naive and uncomplete way of handling non-CGI
                // requests Do not enter this if header is being parsed because
                // we can't know if request will be cgi handled or not at this
                // point
                if (_cgis.find(csockfd) == _cgis.end() &&
                    req.getState() != HTTP::Request::W4_HEADER) {
                    send(csockfd,
                         req.body.str().c_str(),
                         req.body.str().size(),
                         0);

                    // increment before closing connection as it will invalidate
            ...
            if (req.getState() != HTTP::Request::W4_HEADER) {
                ...
                // if request is handled by CGI AND cgi output file descriptor
                // is ready for writing
                if (cgi != _cgis.end()) {
                    // unchunk incoming request body
                    if (req.isChunked() &&
                        req.getState() != HTTP::Request::DONE) {
                        req.parseChunk(buf);
